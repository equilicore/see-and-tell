...
def __directory_images(directory: Union[str, Path], img_extensions: Sequence[str] = None) -> list[Path]:
    """
    Retrieve a list of image files within the specified directory.
    ...
    # and make sure the path is a directory
    directory = directory if os.path.isabs(directory) else Path(os.path.join(HOME, directory))
    # make sure to convert to Path datatype
    directory = Path(directory)
    assert os.path.isdir(directory), "THE PATH PASSED MUST BE A DIRECTORY"
...
def process_save_path(save_path: Union[str, Path, None],
    ...
    if save_path is not None:
        ...
        assert condition is None or condition(save_path), \
